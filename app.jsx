...
- Avoid over-scoring (default bias should be Medium, not High)`;
  ...
  const ALL_COLUMNS = [
    ...
    { key: 'stage1Outcome', label: 'Outcome' },
    { key: 'stage1Freshness', label: 'Review Freshness', essential: true },
    { key: 'stage1Status', label: 'Stage 1 Status', essential: true },
    ...
    { key: 'companyClassification', label: 'Company Class', hiddenByDefault: true },
    { key: 'channel_role', label: 'Role', hiddenByDefault: true },
    { key: 'channel_segment', label: 'Segment', hiddenByDefault: true },
    { key: 'industry', label: 'Industry', hiddenByDefault: true },
    { key: 'category', label: 'Category', hiddenByDefault: true },
    { key: 'displayRevenue', label: '£', hiddenByDefault: true },
    ...
    { key: 'website', label: 'Web', hiddenByDefault: true },
  ...
  const outcomeOptions = window.ProspectToolUtils.OUTCOME_OPTIONS || [];
  const targetQualityOptions = window.ProspectToolUtils.TARGET_QUALITY_OPTIONS || [];
  ...
  const getOutcomeClass = React.useCallback((outcome) => `prospect-pill prospect-pill--outcome prospect-pill--${String(outcome || 'not-contacted').toLowerCase().replace(/\s+/g, '-')}`, []);
  const getTargetQualityClass = React.useCallback((targetQuality) => `prospect-pill prospect-pill--target-quality prospect-pill--${String(targetQuality || 'not-set').toLowerCase().replace(/\s+/g, '-')}`, []);
  ...
  const formatStage1ScoreCell = React.useCallback((score, status) => {
    ...
    return status === 'Partial' ? 'Partial' : 'Unscored';
  }, []);
  ...
  const filtered = React.useMemo(() => rows.filter((r) => {
    ...
    if (filters.stage1Outcome && (r.stage1Outcome || '') !== filters.stage1Outcome) return false;
    if (filters.stage1TargetQuality && (r.stage1TargetQuality || '') !== filters.stage1TargetQuality) return false;
    ...
    if (activePriorityViewKey) {
      ...
      const matchesPriorityView = window.ProspectToolUtils.applyPriorityViewFilter(activePriorityViewKey, [r]).length > 0;
      const matchesLearningView = window.ProspectToolUtils.isProspectInLearningView
        ? window.ProspectToolUtils.isProspectInLearningView(activePriorityViewKey, r)
        : false;
      if (isPriorityView && !matchesPriorityView) return false;
      if (!isPriorityView && !matchesLearningView) return false;
    }
    return true;
  }), [rows, keyword, filters, activePriorityViewKey, builtInPriorityViews]);
      ...
      const av = field === 'stage1score' ? (a.stage1WeightedScore || 0)
          : field === 'stage1tier' ? getStage1TierRank(a)
            ...
            : field === 'lastreviewed' ? (new Date(a.stage1LastReviewed || 0).getTime() || 0)
              : field === 'outcome' ? (window.ProspectToolUtils.getOutcomePriorityValue(a.stage1Outcome || ''))
      ...
      const bv = field === 'stage1score' ? (b.stage1WeightedScore || 0)
          : field === 'stage1tier' ? getStage1TierRank(b)
            ...
            : field === 'lastreviewed' ? (new Date(b.stage1LastReviewed || 0).getTime() || 0)
              : field === 'outcome' ? (window.ProspectToolUtils.getOutcomePriorityValue(b.stage1Outcome || ''))
                : field === 'firstcontact' ? (new Date(b.stage1FirstContactDate || 0).getTime() || 0)
  ...
  const activePriorityLabel = [...builtInPriorityViews, ...builtInLearningViews].find((viewItem) => viewItem.key === activePriorityViewKey)?.label || '';
  ...
  const updateScoringMeta = (key, value) => {
    setEditableScoring((current) => {
      ...
      if (
        key === 'outcome'
        && window.ProspectToolUtils.shouldAutoSuggestFirstContactDate
        && window.ProspectToolUtils.shouldAutoSuggestFirstContactDate(current.outcome, value, current.firstContactDate)
      ) {
        nextScoring.firstContactDate = new Date().toISOString();
      }
  ...
  const updateProspectNextAction = React.useCallback((rowId, nextAction) => {
    ...
    setInlineNextActionDraft('');
    const isPriorityView = builtInPriorityViews.some((viewItem) => viewItem.key === activePriorityViewKey);
    const matchesPriorityView = !activePriorityViewKey || window.ProspectToolUtils.applyPriorityViewFilter(activePriorityViewKey, [updatedRow]).length > 0;
    const matchesLearningView = !activePriorityViewKey || !window.ProspectToolUtils.isProspectInLearningView || window.ProspectToolUtils.isProspectInLearningView(activePriorityViewKey, updatedRow);
    const remainsInView = window.ProspectToolUtils.shouldProspectRemainInCurrentView(updatedRow, isPriorityView ? activePriorityViewKey : '', filters)
      && (isPriorityView ? matchesPriorityView : matchesLearningView);
    setFeedback({
  ...
  }, [applyScoringToRow, persistScoringForRow, activePriorityViewKey, filters, builtInPriorityViews]);
  ...
  const getRowsForView = React.useCallback((viewKey) => {
    const builtInPriority = builtInPriorityViews.find((item) => item.key === viewKey);
    const builtInLearning = builtInLearningViews.find((item) => item.key === viewKey);
    let matchedRows = rows;
    if (builtInPriority) {
      matchedRows = window.ProspectToolUtils.applyPriorityViewFilter(viewKey, rows);
    } else if (builtInLearning && window.ProspectToolUtils.isProspectInLearningView) {
      matchedRows = rows.filter((row) => window.ProspectToolUtils.isProspectInLearningView(viewKey, row));
    }
    if (viewKey === 'my_working_queue') {
      return [...matchedRows].sort(window.ProspectToolUtils.getWorkingQueueSortComparator());
    }
    if (viewKey === 'recently_contacted') {
      return [...matchedRows].sort((a, b) => (new Date(b.stage1FirstContactDate || 0).getTime() || 0) - (new Date(a.stage1FirstContactDate || 0).getTime() || 0));
    }
    return matchedRows;
  }, [rows, builtInPriorityViews, builtInLearningViews]);
  ...
  const applyPriorityView = React.useCallback((priorityViewKey, isActive) => {
    ...
    if (priorityViewKey === 'recently_contacted') {
      setSort('firstcontact_desc');
    }
  }, [sort]);

  const renderCell = (record, key, rowIndex) => {
    const descriptor = [record.channel_segment || record.channel_role, record.country].filter(Boolean).join(' • ') || 'Profile details in drawer';
    ...
    if (key === 'stage1Outcome') return <td data-col={key}><span className={getOutcomeClass(record.stage1Outcome || 'Not Contacted')}>{record.stage1Outcome || 'Not Contacted'}</span></td>;
    ...
    if (key === 'stage1Confidence') return <td data-col={key}>{record.stage1Confidence ? <span className={getConfidenceClass(record.stage1Confidence)}>{record.stage1Confidence}</span> : '—'}</td>;
    ...
    if (key === 'actions') {
      return <td className="cell-actions" data-col={key}><div className="table-row-actions">
    ...
    return <td data-col={key}>{record[key] || '—'}</td>;
  };
    ...
    <div className="ds-card prospect-controls" style={{ display: 'grid', gap: 10 }}>
      ...
      {showPrimaryFilters ? <div className="prospect-filter-group">
        ...
        <div className="filter-grid prospect-filter-grid--primary">
          ...
          <select className="ui-search" value={filters.stage1Outcome} onChange={(e) => setFilters((f) => ({ ...f, stage1Outcome: e.target.value }))}><option value="">Outcome</option>{outcomeOptions.map((v) => <option key={v} value={v}>{v}</option>)}</select>
          <select className="ui-search" value={filters.stage1TargetQuality} onChange={(e) => setFilters((f) => ({ ...f, stage1TargetQuality: e.target.value }))}><option value="">Target quality</option>{targetQualityOptions.map((v) => <option key={v} value={v}>{v}</option>)}</select>
        </div>
    ...
    <div className="saved-views-section ds-card">
      ...
      {showPriorityViews && <div className="saved-views-grid">
        {builtInPriorityViews.map((priorityView) => {
          const matchedRows = getRowsForView(priorityView.key);
          ...
          return <div key={priorityView.key} className={`saved-view-card priority-view-card ${priorityView.key === 'my_working_queue' ? 'priority-view-card--featured' : ''}`.trim()}>
            ...
            <div className="saved-view-actions">
              <button className="ui-btn ui-btn--primary" type="button" onClick={() => {
                ...
                startBulkReview(`Priority View: ${priorityView.label}`, getRowsForView(priorityView.key));
              }}>Review</button>
      ...
      {showPriorityViews && <div className="saved-views-header" style={{ marginTop: 6 }}>
        <h3>Learning &amp; Feedback Views</h3>
        <div className="saved-views-header__actions"><span>Outcome calibration shortcuts</span></div>
      </div>}
      {showPriorityViews && <div className="saved-views-grid">
        {builtInLearningViews.map((learningView) => {
          const matchedRows = getRowsForView(learningView.key);
          const isActive = activePriorityViewKey === learningView.key;
          return <div key={learningView.key} className="saved-view-card priority-view-card">
            <div className="saved-view-head">
              <strong>{learningView.label}</strong>
              {isActive && <span className="top50-badge">Active</span>}
            </div>
            <p>{learningView.description}</p>
            <div className="saved-view-meta">
              <span className="priority-view-card__count">{matchedRows.length}</span>
              <span>matching prospects</span>
            </div>
            <div className="saved-view-actions">
              <button className="ui-btn ui-btn--primary" type="button" onClick={() => startBulkReview(`Learning View: ${learningView.label}`, getRowsForView(learningView.key))}>Review</button>
              <button className="ui-btn ui-btn--secondary" type="button" onClick={() => applyPriorityView(learningView.key, isActive)}>{isActive ? 'Clear view' : 'Apply view'}</button>
            </div>
          </div>;
        })}
      </div>}
      ...
      <aside className="prospect-drawer" role="dialog" aria-modal="true" aria-label={`Partner details for ${selected.displayName}`}>
        <div className="prospect-drawer-header">
          <div className="prospect-drawer-title">
            ...
            <p className="prospect-drawer-descriptor">{[selected.industry, selected.channel_segment || selected.channel_role].filter(Boolean).join(' • ') || 'Prospect profile'}</p>
            <div className="prospect-drawer-subtitle">
              ...
              <span className="score-badge score-badge--stage1">Score {formatStage1ScoreCell(selected.stage1WeightedScore, selected.stage1ScoreStatus)}</span>
              <span className={getNextActionClass(selected.stage1NextAction || 'none')}>{selected.stage1NextAction || 'No next action'}</span>
              <span className={getOutcomeClass(selected.stage1Outcome || 'Not Contacted')}>{selected.stage1Outcome || 'Not Contacted'}</span>
              <span className={`review-freshness review-freshness--${window.ProspectToolUtils.getReviewFreshnessStatus(selected.stage1LastReviewed).toLowerCase().replace(/\s+/g, '-')}`}>{window.ProspectToolUtils.getReviewFreshnessStatus(selected.stage1LastReviewed)}</span>
          ...
          <div className="prospect-drawer-actions">
            ...
            <button type="button" className="prospect-link-chip" aria-label={selected.linkedin ? 'Open LinkedIn' : 'LinkedIn unavailable'} title={selected.linkedin ? 'Open LinkedIn' : 'LinkedIn unavailable'} disabled={!selected.linkedin} onClick={() => selected.linkedin && window.open(window.ProspectToolUtils.normalizeUrl(selected.linkedin), '_blank', 'noopener,noreferrer')}>LinkedIn</button>
            <button type="button" className="prospect-link-chip" aria-label={selected.ch_link ? 'Open Companies House' : 'Companies House unavailable'} title={selected.ch_link ? 'Open Companies House' : 'Companies House unavailable'} disabled={!selected.ch_link} onClick={() => selected.ch_link && window.open(window.ProspectToolUtils.normalizeUrl(selected.ch_link), '_blank', 'noopener,noreferrer')}>Companies House</button>
          ...
          {drawerTab === 'details' && <>
            ...
            <div className="panel-card stage1-scoring-panel">
              <div className="stage1-scoring-panel__head">
                <h4>Evidence &amp; Review Notes</h4>
              </div>
              <p className="stage1-helper-text stage1-helper-text--muted">Capture operational context: next step, confidence and observable evidence.</p>
              <div className="stage1-grid stage1-grid--meta">
                <label className="stage1-field">
                  <span>Next Action</span>
                  <select className="ui-search" value={editableScoring.nextAction || ''} onChange={(event) => updateScoringMeta('nextAction', event.target.value)}>
                  ...
                  {!editableScoring.nextAction && window.ProspectToolUtils.getRecommendedNextAction(editableScoring) && <small>Recommended (editable): {window.ProspectToolUtils.getRecommendedNextAction(editableScoring)}</small>}
                </label>
              </div>
              <div className="stage1-summary-grid">
            ...
            <div className="panel-card stage1-scoring-panel">
              ...
              <p className="stage1-helper-text stage1-helper-text--muted">Use Outcome and Target Quality to track what happened after Stage 1 scoring.</p>
              <div className="stage1-grid stage1-grid--meta">
                <label className="stage1-field">
                  <span>Outcome</span>
                  <select className="ui-search" value={editableScoring.outcome || ''} onChange={(event) => updateScoringMeta('outcome', event.target.value)}>
                ...
                <label className="stage1-field">
                  ...
                  <input className="ui-search" type="date" value={(editableScoring.firstContactDate || '').slice(0, 10)} onChange={(event) => updateScoringMeta('firstContactDate', event.target.value ? new Date(`${event.target.value}T00:00:00.000Z`).toISOString() : '')} />
                </label>
                <label className="stage1-field">
                  <span>Target Quality</span>
                  <select className="ui-search" value={editableScoring.targetQuality || ''} onChange={(event) => updateScoringMeta('targetQuality', event.target.value)}>
                    <option value="">Not set</option>
                    {targetQualityOptions.map((option) => <option key={option} value={option}>{option}</option>)}
                  </select>
                </label>
              </div>
              <div className="stage1-summary-grid">
                <div><span>Current Outcome</span><strong><span className={getOutcomeClass(editableScoring.outcome || 'Not Contacted')}>{editableScoring.outcome || 'Not Contacted'}</span></strong></div>
                <div><span>First Contact</span><strong>{window.ProspectToolUtils.formatFirstContactDate ? window.ProspectToolUtils.formatFirstContactDate(editableScoring.firstContactDate) : '—'}</strong></div>
                <div><span>Target Quality</span><strong>{editableScoring.targetQuality ? <span className={getTargetQualityClass(editableScoring.targetQuality)}>{editableScoring.targetQuality}</span> : 'Not set'}</strong></div>
              </div>
            </div>

            <div className="panel-card">
              <h4>Company details</h4>
              <div className="prospect-metadata-grid">
                <p><span>Company Class</span>{selected.companyClassification || 'Unclassified'}</p>
                <p><span>Type</span>{selected.category || '—'}</p>
                <p><span>Region</span>{selected.country || '—'}</p>
                <p><span>Industry</span>{selected.industry || '—'}</p>
                <p><span>Status</span>{selected.trading_status || '—'}</p>
                <p><span>Channel Role</span>{selected.channel_role || '—'}</p>
                <p><span>Channel Segment</span>{selected.channel_segment || '—'}</p>
                <p><span>Adopter Profile</span>{selected.adopter_profile || '—'}</p>
                <p><span>Employees</span>{selected.displayEmployees}</p>
                <p><span>Revenue</span>{selected.displayRevenue}</p>
                <p><span>Location</span>{selected.displayLocation} {selected.postcode}</p>
              </div>
...
function PartnerTiersGovernancePage() {
  ...
  const visualHighlights = [
    { label: "Legacy top tier", value: "40%", note: "Old Platinum cap" },
    { label: "Reseller upside", value: "45%", note: "New Platinum base margin" },
    { label: "MSP upside", value: "60%", note: "Platinum MSP / Service Provider" },
    { label: "Deal uplift", value: "+3–5%", note: "On qualified registered deals" },
  ];

  const storySteps = [
    {
      title: "Create clearer separation",
      text: "Differentiate advisory, reseller, and MSP motions so the commercial model fits the way each partner actually operates.",
    },
    {
      title: "Reward the right behaviour",
      text: "Increase upside for partners that invest in pipeline, enablement, service capability, and customer ownership.",
    },
    {
      title: "Protect delivery quality",
      text: "Hold MSP economics and white-label rights behind capability validation, support readiness, and operational maturity.",
    },
  ];

  const rampJourney = [
    { stage: "1", title: "Assess fit", text: "Validate business plan, capability, and growth potential." },
    { stage: "2", title: "Enable", text: "Complete accreditation, onboarding, and MSP readiness checks where required." },
    { stage: "3", title: "Prove traction", text: "Build pipeline, progress deals, and show consistent engagement." },
    { stage: "4", title: "Confirm tier", text: "Lock in tier once milestones are met, or adjust if they are not." },
  ];

  const trackSummary = [
    { badge: "consultant", stat: "Up to 20%", label: "Influence commission", note: "Advisory route only" },
    { badge: "reseller", stat: "22% → 45%", label: "Accredited to Platinum", note: "Commercial ownership with IPI delivery support available" },
    { badge: "msp", stat: "40% → 60%", label: "Silver to Platinum", note: "Higher margin for validated delivery and support ownership" },
  ];
    ...
    <div className="page-shell">
      <div className="content-shell">
        <PageHeader
          ...
          subtitle="A more visual commercial story that shows how IPI separates partner motions, rewards the right behaviour, and gives partners a clearer path to grow."
          right={<span className="pill">Governance section</span>}
        />

        <SectionCard style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "24px clamp(20px,3vw,32px)", display: "grid", gap: 22 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16, alignItems: "stretch" }}>
              <div className="panel-card" style={{ background: "linear-gradient(145deg,rgba(103,216,255,0.18),rgba(255,255,255,0.03))", borderColor: "rgba(103,216,255,0.22)", display: "grid", gap: 14 }}>
                <div>
                  <div className="mini-kicker">At a glance</div>
                  <h2 style={{ margin: "4px 0 8px", color: "#F4FAFF", fontSize: 30, lineHeight: 1.05 }}>A partner model built around real operating maturity.</h2>
                  <p style={{ margin: 0, color: "#D7E6EF", lineHeight: 1.7, maxWidth: 720 }}>
                    The story stays the same: IPI wants to be easy to work with, commercially compelling, and protective of customer outcomes. The difference is that the page now shows that story faster.
                  </p>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12 }}>
                  {visualHighlights.map((item) => (
                    <div key={item.label} style={{ padding: 14, borderRadius: 16, background: "rgba(9,15,29,0.38)", border: "1px solid rgba(255,255,255,0.08)" }}>
                      <div style={{ color: "#9BB4C7", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700 }}>{item.label}</div>
                      <div style={{ marginTop: 8, color: "#F4FAFF", fontSize: 28, lineHeight: 1, fontFamily: "'Syne',sans-serif" }}>{item.value}</div>
                      <div style={{ marginTop: 6, color: "#C9D9E3", fontSize: 12, lineHeight: 1.5 }}>{item.note}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="panel-card" style={{ display: "grid", gap: 12 }}>
                <div className="mini-kicker">Core message</div>
                {storySteps.map((step, index) => (
                  <div key={step.title} style={{ display: "grid", gridTemplateColumns: "30px 1fr", gap: 12, alignItems: "start" }}>
                    <div style={{ width: 30, height: 30, borderRadius: 999, background: "rgba(212,168,67,0.18)", border: "1px solid rgba(212,168,67,0.28)", color: "#F6D58B", display: "grid", placeItems: "center", fontWeight: 800 }}>{index + 1}</div>
                    <div>
                      <div style={{ color: "#F4FAFF", fontWeight: 800 }}>{step.title}</div>
                      <div style={{ color: "#C9D9E3", fontSize: 13, lineHeight: 1.65 }}>{step.text}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 14 }}>
              {trackSummary.map((item) => (
                <div key={item.label} className="panel-card" style={{ background: "linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))" }}>
                  <span style={pillStyle(item.badge)}>{PARTNER_COMMERCIAL_TRACK_BADGES[item.badge].label}</span>
                  <div style={{ marginTop: 12, color: "#F4FAFF", fontFamily: "'Syne',sans-serif", fontSize: 30, lineHeight: 1 }}>{item.stat}</div>
                  <div style={{ marginTop: 6, color: "#F4FAFF", fontWeight: 700 }}>{item.label}</div>
                  <p style={{ margin: "8px 0 0", color: "#C9D9E3", fontSize: 13, lineHeight: 1.6 }}>{item.note}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={sharedSectionStyles}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 14 }}>
            {rampJourney.map((step) => (
              <SectionCard key={step.title} style={{ background: "linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))" }}>
                <div style={{ width: 38, height: 38, borderRadius: 14, background: "rgba(103,216,255,0.14)", border: "1px solid rgba(103,216,255,0.24)", color: "#67D8FF", display: "grid", placeItems: "center", fontWeight: 800, fontFamily: "'Syne',sans-serif" }}>{step.stage}</div>
                <h3 style={{ margin: "10px 0 6px", color: "#F4FAFF", fontSize: 18 }}>{step.title}</h3>
                <p style={{ margin: 0, color: "#C9D9E3", fontSize: 13, lineHeight: 1.65 }}>{step.text}</p>
              </SectionCard>
            ))}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 14 }}>
            ...
            <SectionCard>
              <div className="mini-kicker">What is reassessed</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 10 }}>
                {rampChecks.map((item) => (
                  <div key={item} style={{ padding: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(8,13,24,0.32)", color: "#D9E7F0", fontSize: 13, fontWeight: 600 }}>{item}</div>
                ))}
              </div>
            </SectionCard>
          </div>
        ...
        <section style={sharedSectionStyles}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 14 }}>
            <SectionCard style={{ background: "linear-gradient(135deg,rgba(103,216,255,0.14),rgba(255,255,255,0.03))", borderColor: "rgba(103,216,255,0.22)" }}>
              <div className="mini-kicker">Reseller ladder</div>
              <div style={{ color: "#67D8FF", fontFamily: "'Syne',sans-serif", fontSize: 28, lineHeight: 1 }}>22% → 45%</div>
              <p style={{ margin: "8px 0 0", color: "#D9E7F0", lineHeight: 1.7 }}>Accredited to Platinum keeps the entry point accessible while creating stronger upside at the top end.</p>
            </SectionCard>
            <SectionCard style={{ background: "linear-gradient(135deg,rgba(212,168,67,0.14),rgba(255,255,255,0.03))", borderColor: "rgba(212,168,67,0.24)" }}>
              <div className="mini-kicker">MSP ladder</div>
              <div style={{ color: "#F6D58B", fontFamily: "'Syne',sans-serif", fontSize: 28, lineHeight: 1 }}>40% → 60%</div>
              <p style={{ margin: "8px 0 0", color: "#D9E7F0", lineHeight: 1.7 }}>Higher economics sit behind delivery ownership, support maturity, and validated service capability.</p>
            </SectionCard>
            <SectionCard style={{ background: "linear-gradient(135deg,rgba(212,168,67,0.14),rgba(255,255,255,0.03))", borderColor: "rgba(212,168,67,0.24)" }}>
              <div className="mini-kicker">MSP Enablement Phase</div>
              <p style={{ margin: 0, color: "#D9E7F0", lineHeight: 1.8 }}>Before a partner is presented as MSP / Service Provider, IPI validates staff accreditation, support readiness, service wrap definition, and operational readiness to deliver as an MSP.</p>
            </SectionCard>
          </div>
          <SectionCard style={{ padding: 0, overflow: "hidden" }}>
...
const MARKET_VISION_DATA = {
  ...
  valueCards: [
    { icon: "🌟", title: "Better customer journeys", detail: "Connected channels and context create smoother, lower-effort experiences." },
    { icon: "⚡", title: "Faster agent productivity", detail: "AI guidance and automation reduce effort and speed up resolution times." },
    { icon: "💸", title: "Lower cost to serve", detail: "Operational efficiency and smarter workflows reduce service delivery overhead." },
    { icon: "🛡️", title: "Stronger security and compliance", detail: "Built-in controls protect customer data and simplify regulatory alignment." },
    { icon: "☁️", title: "More flexible cloud operations", detail: "Cloud-native tooling improves resilience, scalability and deployment agility." },
    { icon: "🧩", title: "Scalable modern workplace support", detail: "Managed digital workplace services keep teams secure, productive and supported." },
  ],
};
    ...
    <div className="page-shell market-vision-page">
      ...
      <SectionWrapper>
        ...
        <div className="market-value-grid">
          {MARKET_VISION_DATA.valueCards.map((value) => (
            <StandardCard className="market-value-card" key={value.title}>
              <div className="market-value-card__icon" aria-hidden="true">{value.icon}</div>
              <h3>{value.title}</h3>
              <p>{value.detail}</p>
            </StandardCard>
          ))}
