...
const PARTNER_MESSAGING_THEMES = [
  ["Start anywhere. Expand everywhere.", "Partners can lead with any module and grow into a complete customer experience estate over time."],
  ["More than CCaaS", "Most vendors sell one product. IP Integration enables partners to build complete CX solutions."],
  ["One partner ecosystem, multiple revenue streams", "Blend platform licensing, services and managed support for stronger recurring value."],
  ["Expert support from sale to service", "From pre-sales architecture to delivery and lifecycle success, partners are supported end-to-end."],
  ["Enterprise-ready trust and governance", "Reassure enterprise buyers with transparent legal, security, compliance and governance resources."],
].map(([title, desc]) => ({ title, desc }));

const MOCKUP_TABS = [
  { id: "landing", label: "Landing Page" },
  { id: "program", label: "Program Page" },
  { id: "trust", label: "Trust Page" },
];

function ChannelMarketingPage() {
  const [mockView, setMockView] = React.useState("landing");
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", paddingBottom: 40 }}>
        ...
        <SectionWrapper>
          <SectionHeader title="Core Messaging Framework" />
          <div className="cm-two-col">
            <StandardCard>
              <h3 className="cm-subhead">Primary Positioning</h3>
              <HighlightCard className="cm-inline-highlight">
                <div className="cm-featured-statement">Build profitable CX solutions with IP Integration.</div>
                <p>Sell telephony, UC, CCaaS, AI, secure payments and managed services from one partner ecosystem. Start with any solution, then expand customer value and recurring revenue over time.</p>
              </HighlightCard>
            </StandardCard>
            <StandardCard>
              <h3 className="cm-subhead">Messaging Themes</h3>
              <div className="cm-theme-stack">
                {PARTNER_MESSAGING_THEMES.map((theme) => (
                  <div key={theme.title} className="cm-theme-card">
                    <h4>{theme.title}</h4>
                    <p>{theme.desc}</p>
                  </div>
                ))}
        ...
        <SectionWrapper>
          <SectionHeader title="Partner Website Concept" description="Interactive native concept prototype for a future partner-facing experience." />
          <div className="cm-mock-wrap">
            <div className="cm-mock-topbar">
              <div className="cm-dots"><span /><span /><span /></div>
              <div className="cm-url">https://ipintegration.co.uk/partners</div>
            </div>
            <div className="cm-mock-controls">
              {MOCKUP_TABS.map((tab) => (
                <button key={tab.id} type="button" className={`cm-tab-btn ${mockView === tab.id ? "active" : ""}`} onClick={() => setMockView(tab.id)}>
                  {tab.label}
                </button>
              ))}
            </div>
              ...
              {mockView === "landing" && (
                <React.Fragment>
                  <div className="cm-mock-hero">
                    <h3>Grow your CX revenue with IP Integration</h3>
                    <p>Partner with a vendor that lets you sell telephony, UC, contact centre, AI, secure payments and managed services as point solutions or a full customer experience stack.</p>
                    <div className="cm-mock-cta"><StandardButton>Become a Partner</StandardButton><SecondaryButton>Explore the Program</SecondaryButton></div>
                    <div className="cm-proof-bar">{["Modular portfolio", "Recurring revenue", "Sales & technical support", "Enterprise ready"].map((p) => <span key={p}>{p}</span>)}</div>
                  </div>
                  <div className="cm-grid-4">
                    {["Complete CX portfolio", "Start with any entry point", "Services that reduce delivery risk", "Grow beyond one product sale"].map((item) => (
                      <div key={item} className="cm-theme-card"><h4>{item}</h4></div>
                    ))}
                  </div>
                  <StandardCard>
                    <h4 className="cm-journey-title">Revenue Journey</h4>
                    <div className="cm-revenue-row">{["SIP / Telephony", "UCaaS", "CCaaS", "AI & Automation", "Secure Payments", "Managed Services"].map((step) => <span key={step}>{step}</span>)}</div>
                    <p>Partners can land with any solution and expand into broader, higher-value customer outcomes over time.</p>
                  </StandardCard>
                  <div className="cm-final-cta"><h4>Ready to build with IP Integration?</h4><div><StandardButton>Apply Now</StandardButton><SecondaryButton>Book a Discovery Call</SecondaryButton></div></div>
                </React.Fragment>
              )}
              {mockView === "program" && (
                <React.Fragment>
                  <div className="cm-grid-3">
                    {[
                      ["Registered", "Entry tier for partners launching their first opportunities.", "Benefits: sales kit, portal access", "Commitment: onboarding + first opportunity plan"],
                      ["Silver", "Growth tier for partners building repeat pipeline.", "Benefits: enhanced margins, pre-sales support", "Commitment: quarterly plan + certified contacts"],
                      ["Gold", "Strategic tier for scaled partner-led delivery.", "Benefits: highest benefits, MDF priority, exec sponsorship", "Commitment: annual targets + joint governance"],
                    ].map(([tier, desc, benefit, commitment]) => (
                      <StandardCard key={tier} className="cm-big-card"><h3>{tier}</h3><p>{desc}</p><p>{benefit}</p><p>{commitment}</p></StandardCard>
                    ))}
                  </div>
                  <div className="cm-grid-4">
                    {["Sales Support", "Pre-Sales Design", "Delivery Services", "Ongoing Partner Success"].map((item) => (
                      <div key={item} className="cm-theme-card"><h4>{item}</h4></div>
                    ))}
                  </div>
                  <StandardCard><p>Program commitments are transparent and aligned to growth stage so partners can scale at pace with predictable support.</p></StandardCard>
                </React.Fragment>
              )}
              {mockView === "trust" && (
                <React.Fragment>
                  <HighlightCard>
                    <h3 className="cm-highlight-title">Partner Trust Portal</h3>
                    <p className="cm-highlight-copy">Governance-first experience with clear legal, security, compliance and product documentation in one place.</p>
                  </HighlightCard>
                  <StandardCard>
                    <div className="cm-grid-3">
                      {["Partner Agreement", "Product Schedules", "Security Overview", "Compliance Documents", "Pricing & Commercial Docs"].map((doc) => (
                        <div key={doc} className="cm-doc-item">⬇ {doc}</div>
                      ))}
                    </div>
                  </StandardCard>
                  <StandardCard><p>Trust content reinforces enterprise-grade assurance and reduces procurement friction for partners and end customers.</p></StandardCard>
                </React.Fragment>
              )}
            </div>
          </div>
        ...
        <SectionWrapper>
          <SectionHeader title="Recommended Website Calls to Action" />
          <div className="cm-grid-3">
            {[
              ["Become a Partner", "Primary conversion CTA for hero sections and value proposition pages where intent is highest."],
              ["Book a Discovery Call", "Use when prospects need strategic consultation before submitting an application."],
              ["Explore the Partner Program", "Ideal for mid-funnel users evaluating tiers, benefits and commitments."],
              ["Download Partner Overview", "Supports quick internal sharing by reseller decision makers and leadership teams."],
              ["View Trust Documents", "Place near procurement-sensitive content to accelerate due diligence and legal review."],
              ["Speak to Channel Team", "Use on high-intent pages where immediate human support can unlock conversion."],
            ].map(([title, copy]) => (
              <StandardCard key={title} className="cm-big-card"><h3>{title}</h3><p>{copy}</p></StandardCard>
            ))}
          </div>
        </SectionWrapper>
        ...
        <SectionWrapper>
          ...
          <div className="channel-stack">
            {[
              "Create a dedicated Partners top-level website area",
              "Build the partner landing page first",
              "Add a clear revenue opportunity section",
              "Publish partner program structure and benefits",
              "Launch a trust and compliance portal",
              "Add a simple partner application flow",
              "Use partner proof points and case studies",
              "Align website messaging with sales outreach and partner recruitment",
            ].map((task) => (
              <div key={task} className="channel-card channel-action-card">
                <span>☐ {task}</span>
                <span className="channel-chip">Planned</span>
              </div>
            ))}
          </div>
        </SectionWrapper>
